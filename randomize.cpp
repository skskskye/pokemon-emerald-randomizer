...
const uint32_t routeOneOffset = 0x5507E6; // temp for now 
...
std::vector<Pokemon> randomizeStarters(FileIO& game) {
    ...
    uint32_t current = starterOffset;
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        */
        ...
        game.writeToFile((current + (i * 2)) , randomPokemon);
...
void randomizeRoutes (FileIO &game) {
    ...
    uint32_t current = routeOneOffset;
    uint32_t previous = 0; 
    int count = 0;
    ...
    for (int i = 0; i < 2; i++ ) { 
        ...
        while(1) { 
            ...
            uint8_t* currentBytes = game.readByte(current, 4);
            ...
            for (int i = 0; i < 2; i++) { 
                ...
                //std::cout << "current[" << i << "]: " <<  (long)currentBytes[i];
                
                if(currentBytes[i] != valueToEqual) {
                    found = false; 
                    break;
                } else { 
                    found = true;
            ...
            game.writeToFile(current , randomPokemon); 
